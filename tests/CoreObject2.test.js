...
describe("event objesi",function(){
    it("obje oluşturuldu ise",function(){
        let event = new CoreObject();
    });
});
describe("emit parametre gönderme",function(){
    const event =new CoreObject();
    const testObject=new Test("variable 1");
    before(async function(){
        await event.on("message",function func(param){
            param.setRandomVariable("variable 2");
        });
        await event.emit("message",testObject);
...
describe("emit çoklu fonksiyon",function(){
    ...
    const testObject3=new Test(0);
    before(async function(){
        await event.on("click",function(){
...
describe("on ve once",function(){
    ...
    before(async function(){
        ...
        await event.once("click",function fonk2(){
...
describe("emit callback",function(){
    ...
    before(async function(){
        await event.on("click",function fonk(param){
    ...
    it ("emit'e callback parametresi verildi",function(){
